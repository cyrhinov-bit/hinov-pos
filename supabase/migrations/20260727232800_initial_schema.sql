...
BEGIN
    ...
    IF NEW.return_number IS NULL OR NEW.return_number = '' THEN
        NEW.return_number := 'RET-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || LPAD(SUBSTRING(gen_random_uuid()::text, 1, 8), 8, '0');
    END IF;
...
BEGIN
    v_return_number := 'RET-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || LPAD(SUBSTRING(gen_random_uuid()::text, 1, 8), 8, '0');
